...
**新同事接入流程(零技术门槛)**:
1. 打开平台 → 最右「💬 定制」tab → 输入访问口令(自己粘 API Key 的免口令)
2. **⚡ 快速定制(默认)**:点一个示例或填 4 格(条线/地域语言/行业/不能漏什么)
   → 「⚙️ 一键生成配置」,1-2 次调用直接出结果;复杂需求或想微调再切「💬 对话定制」
3. 应用自动逐源实测,失败的自动修一轮
4. 预览满意 → 「🚀 提交审核」→ 自动在 GitHub 开 PR
5. 你在 GitHub 看一眼 diff → Merge → 1-2 分钟后 TA 的专属链接生效:
   `xxx.streamlit.app/?profile=名字`

**token 防意外消耗(三重护栏)**:访问口令 + 每会话 40 轮上限 +
**全站每日预算硬停**(默认 30 万 tokens/天 ≈ 最多 $2-3;超限当天停用、
次日自动恢复;Secrets 里 `INTERVIEW_DAILY_BUDGET = "500000"` 可调)。
定制 tab 顶部实时显示当日已用量。
