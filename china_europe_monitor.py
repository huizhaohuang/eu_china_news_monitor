"""
News Monitor —— 中欧起家、现服务整个多条线 desk 的新闻监测台
============================================================
一套部署,两种用法:

  不带参数         默认监测台(中欧地缘政治+产经):71 源 / 9 个主题 tab,行为被
                   黄金基线测试逐字锁死(tests/test_baseline.py)。
  ?setup=1         三层筛选流(区域 → 领域 → 口径),记者 60 秒配出专属监控页。
  ?profile=p3.xxx  个性化页。配置全在 URL 里(≤600 字符),加书签即保存、
                   发链接即分享,服务端零落盘(公私边界见 README)。
  ?lang=en         英文 UI(只翻界面,不动新闻内容)。

数据资产(均在仓库,与本文件解耦)
--------------------------------
  sources.json          默认台源清单(首次运行由 DEFAULT_SOURCES 播种,之后侧栏管理)
  source_registry.json  定制系统源注册表:415 源 × 区域 × 口径 × 条线三维打标
  packs.json            16 个条线词表包(+ 记者画像预设)
  source_library.json   领域扩展库(访谈式配置的候选目录)

管线(抓取 → 展示)
------------------
  并发抓取(每源独立超时,单源挂起不阻塞整体)
    → 相关性闸门(欧洲流须提中国 / 中方流须提欧洲)
    → 归类打分(kw=1 / 实体=3,标题×2,≥2 入类;多标签)
    → 链接归一去重 → gnews 跳转链解码(大陆可直达,gnews_decoder)
    → 陈年新闻防御(识破 Google 给旧文盖的新时间戳,stale_guard)
    → 三级同题聚类:词集 Jaccard → idf 加权跨媒体 → 灰带 LLM 裁决(event_merge)
    → 低质媒体降权(DEMOTED_OUTLETS:不当代表、不计独立报道数)

同级模块:beat_packs(筛选流) profile_code(配置码) i18n(双语)
         feedback(反馈) events_monitor(活动雷达,当前隐藏)

Run:  streamlit run china_europe_monitor.py
Test: python -m pytest tests/          (SKIP_E2E=1 跳过联网 e2e,CI 即此)
...
# ---------------------------------------------------------------------------
# 1. 默认台源配置 —— 首次运行播种 sources.json,此后完全由侧边栏管理
#    (定制系统的 415 源在 source_registry.json,不走这里)
...
# ---------------------------------------------------------------------------
# 2. 默认台归类体系(9 类,四语关键词+实体表)与优先级规则
#    (条线包的词表在 packs.json;二者结构相同,经同一套 _nt 规范化)
...
# ---------------------------------------------------------------------------
# 个性化机制(两条路,都在此段):
#   A. ?profile=p3.xxx —— 三层筛选流生成的配置码,零落盘(现主力,见 apply_packs_config)
#   B. ?profile=名字   —— 目录档案 profiles/<名字>/(历史机制,该目录未启用)
# 不带参数 = 默认档案(原中欧台,行为零变化)。
# 注:两个 apply_* 都改模块级全局。**这是安全的**——Streamlit 每次 rerun 会把本
# 脚本从头执行一遍,全局随之重置(已实测:p3 会话之后的默认会话不受污染)。
# 但该安全性依赖"词表字面量在会被重跑的主脚本里":若日后把词表移入 import 的
# 模块加载,必须同时改成显式传参,否则会变成真正的跨会话串档 bug。
...
# ---------------------------------------------------------------------------
# 3. 抓取 + 规范化 + 去重聚类
#    线程池并发,每源独立超时;去重/解码/防陈年/三级聚类的顺序见文件头管线图
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5. UI —— 自上而下执行:语言 → 个性化解析 → ?setup 分流 → 侧栏 → 抓取 → 标签页
# ---------------------------------------------------------------------------
